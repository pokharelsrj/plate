...
// WorkoutStatsPage renders the lifting-analytics dashboard shell.
func WorkoutStatsPage(w http.ResponseWriter, r *http.Request) {
	components.WorkoutStatsPage(parseRange(r)).Render(r.Context(), w)
}

// WorkoutStatsData serves the analytics payload as JSON for the web page's charts.
func WorkoutStatsData(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(buildWorkoutStatsPayload(parseRange(r)))
}

// APIWorkoutStats serves the same payload over the X-Api-Key API for the iOS app.
